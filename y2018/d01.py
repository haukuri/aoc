...
def main():
    import pathlib
    input_file = pathlib.Path(__file__).parent / 'd01input'
    changes = [int(l) for l in input_file.open().readlines()]
    shift = sum(changes)
    print('Frequency shift:', shift)
    
    frequencies = set()
    f = 0
    frequencies.add(f)
    for df in cycle(changes):
        f += df
        if f in frequencies:
            print('First revisited frequency:', f)
            break
        frequencies.add(f)
